                ...
                if(code === 0){
                    ...
                    setTimeout(async () => {
                        ...
                        for(let value of hosts){
                            ...
                            if(value === '127.0.0.1'||value === '127.0.0.2'||value === '127.0.0.3'||value === '127.0.0.4'){
                                assert.equal(cnt, 3, 'Node '+ value + ' is not balanced');
                            }else {
                                 assert.equal(cnt, 0, 'Node '+ value + ' is not balanced');
                            }
                        }
                        console.log("Nodes are all load Balanced on 4 nodes matching with placement info mentioned in topology key with preference value 1.")
                            ...
                            stopOneNode.on('close', async (code) => {
                               if(code === 0){
                                ...
                                setTimeout(() => {
                                    ...
                                    for(let value of hosts){
                                        let cnt = connectionMap.get(value);
                                        if(value === '127.0.0.1' || value === '127.0.0.2' || value === '127.0.0.4'){
