...
//Testing load balancing when second preference value zone is given as cloud.datacenter.* 
async function createConnection(){
    const yburl = "postgresql://yugabyte:yugabyte@127.0.0.1:5433/yugabyte?loadBalance=true&&topologyKeys=cloud1.datacenter2.*:2,cloud1.datacenter1.rack1:1"
...
function example(){
    ...
    destroyCluster.on('close', async (code) => {
        if(code === 0){
            const createCluster = spawn("./bin/yb-ctl", ["create", "--rf", "3", "--placement_info", "cloud1.datacenter1.rack1,cloud1.datacenter2.rack1,cloud1.datacenter2.rack2"]);
            ...
            createCluster.on('close', async (code) => {
                if(code === 0){
                    ...
                    console.log("Testing load balancing when second preference value zone is given as cloud.datacenter.*")
