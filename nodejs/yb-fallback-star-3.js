...
async function createConnection(){
    const yburl = "postgresql://yugabyte:yugabyte@127.0.0.1:5433/yugabyte?loadBalance=true&&topologyKeys=cloud1.datacenter2.rack3:1,cloud1.datacenter1.rack3:1,cloud1.datacenter1.rack1:2,cloud1.datacenter2.rack2:2"
                ...
                if(code === 0){
                    ...
                    setTimeout(async () => {
                        ...
                        for(let value of hosts){
                            ...
                            if(value === '127.0.0.7'||value === '127.0.0.4'){
                                assert.equal(cnt, 6, 'Node '+ value + ' is not balanced');
                            }else {
                                 assert.equal(cnt, 0, 'Node '+ value + ' is not balanced');
                            }
                        }
                        console.log("Connections are all load Balanced on 2 nodes matching with placement info mentioned in topology key with preference value 1.")
                        ...
                        setTimeout(async() => {
                            ...
                            const stopOneNode = spawn("./bin/yb-ctl", ["stop_node", "4"])
                            stopOneNode.stdout.on("data", () => {
                                console.log("Stopping node with host IP - 127.0.0.4")
                            })
                            stopOneNode.on('close', async (code) => {
                               if(code === 0){
                                clientArray = await createNumConnections(numConnections)
                                setTimeout(() => {
                                    console.log(numConnections, "connections are created after stopping node 4.");
                                    let connectionMap = pg.Client.connectionMap;
                                    console.log("Connection Map: ", connectionMap)
                                    const hosts = connectionMap.keys();
                                    for(let value of hosts){
                                    ...
                                    console.log("All connections are load balanced to one server.")
